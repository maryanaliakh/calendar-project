...
export function NoteProvider({ children }) {
    ...
    useEffect(() => {
        fetch(`${import.meta.env.VITE_API_URL}/notes`)//for Docker backend without Docker localhost
            .then(res => res.json())
    ...
    const getDataFromForm = (month, day, contentNote) => {
        ...
        return fetch(`${import.meta.env.VITE_API_URL}/notes/${month}/${day}`, {
            method: "POST",
            ...
            .then(data => {
                return fetch(`${import.meta.env.VITE_API_URL}/notes`)
                    .then(res => res.json())
                    .then(allNotes => {
                        console.log("Data from server:", allNotes);
                        setNotes(allNotes);
                    });
            })
            .catch(err => console.error(err));
